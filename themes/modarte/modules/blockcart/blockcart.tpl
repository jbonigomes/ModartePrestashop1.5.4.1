...
{/foreach}
		...
		<p id="cart-buttons">
			...
			<a href="{$link->getPageLink("$order_process", true)}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart'}" rel="nofollow"><i class="icon-gift"></i> {l s='Check out' mod='blockcart'}</a>
		</p>
	</div>
	</div>
</div>
